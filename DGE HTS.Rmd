...
srr <- unlist(lapply(gsm_list, function(gsm) {
  rel <- Meta(gsm)$relation
  rel <- rel[grepl("SRA", rel)]
  str_extract(rel, "SRR[0-9]+")
}))

srr <- unique(na.omit(srr))
writeLines(srr, "data/fastq/SRR_list.txt")
