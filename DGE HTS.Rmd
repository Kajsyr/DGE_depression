...
conda activate sratool #activate the chosen conda environment

  fasterq-dump SRR28203261 \
    --split-files \
    --threads 8
...
```{r}
gse <- getGEO("GSE260603", GSEMatrix = FALSE)
...
cmd <- paste(
  "esearch -db sra -query GSE260603 |",
  "efetch -format runinfo"
)

runinfo <- read_csv(pipe(cmd), show_col_types = FALSE)

srr <- unique(runinfo$Run)
length(srr)

writeLines(srr, "SRR_list.txt")
...
```
```{r}
write.csv(annot_over_genes, file="data/overexpressed_genes.csv")
write.csv(dge_res_df, file="data/dge_res_df.csv")
```

# GO anylisis

```{r}
gene_universe <- row.names(dge_res_df)
```

```{r}
library(dplyr)
filtered_genes <- dge_res_df %>% filter(diffexpressed !="NO") %>% row.names()
filtered_genes
```
...
GO_terms <-  enrichGO(gene = filtered_genes, #genes of interest
                      universe = gene_universe, # all detected genes
                      OrgDb = "org.Hs.eg.db", #species-specific annotation package
                      pAdjustMethod = "BH", #Benjamini-Hochberg correction for multiple testing
                      pvalueCutoff = 0.01,
                      qvalueCutoff = 0.01, #thresholds for reporting the outcomes
                      readable = T, # adding gene names
                      keyType = "ENSEMBL", #the format of provided genes
                      ont = "CC") # "Biological Processes" - GO-terms ontology - alternatives: Molecular Functions (MF) and Celular Components (CC) 
...
write_tsv(to_save, file = "data/sumo/immune_markers.tsv")

to_PCA <- imune_marker %>% dplyr::select(!c(ID, age, sex, BMI, status, analysis_type, batch_variable))
to_PCA
```
...
#Run PCA
to_PCA_clean <- na.omit(to_PCA)
pca <- prcomp(to_PCA_clean, scale. = TRUE)
...
sumo prepare -plot LAML.png data/sumo/immune_markers.tsv output.npz
