...
```{r}
#Rename columns and extract nedeed
#Set new name
...
# Mapowanie ENSEMBL na chromosomy za pomocą nowych nazw kolumn (TXCHROM)
chr_map <- AnnotationDbi::select(
  txdb,
  keys = rownames(count_matrix_raw),
  keytype = "ENSEMBL",
...
# Filtrowanie (z uwzględnieniem, że TxDb używa formatu "chrY")
genes_Y <- chr_map$ENSEMBL[chr_map$TXCHROM == "chrY"]
genes_Y <- na.omit(unique(genes_Y))

count_matrix <- count_matrix_raw[!rownames(count_matrix_raw) %in% genes_Y, ]
...
```{r}
background_genes <- data_normalized %>% select(Geneid)
rm(backgroud_genes)
```
...
```{r}
results <- lapply(imune_marker_numeric, shapiro.test)
results
```

```{r}
sum(is.na(imune_marker_numeric))
```
...
write_tsv(label, "sumo/labels_affected.tsv")
write_tsv(label, "sumo/labels_gender.tsv")
```
...
conda activate SUMO
cd sumo
sumo evaluate LAML/k7/clusters.tsv labels_affected.tsv
cd ..
conda deactivate #deactivate the environment
```
How to inerprate?
